...
//        PHPExcel_Worksheet_PageSetup
    ...
    public function renderBody($activeSheet){
        /** @var ThucHienCongViec $data */
        $data = $this->data;
        $activeSheet->setCellValue("B5", "Người đề xuất: ".$data->user->hoten);
        $activeSheet->setCellValue("B6", "Bộ phận/phòng ban/Công ty: ".$data->phongBan->name);
        $activeSheet->setCellValue("A9", $data->tieu_de."\n".$data->noi_dung);
        $activeSheet->setCellValue("B10", $data->user->hoten);
        $activeSheet->setCellValue("D10", "Ngày: ".date("d/m/Y", strtotime($data->created)));
        $trangThaiCongViec = TrangThaiThucHienCv::find()
            ->andFilterWhere(['thuc_hien_cong_viec_id' => $data->id])
            ->orderBy(['id' => SORT_DESC])
            ->one();
        /** @var $trangThaiCongViec TrangThaiThucHienCv */
        if(!is_null($trangThaiCongViec)){
            $activeSheet->setCellValue("A13", $trangThaiCongViec->trang_thai."\n".$trangThaiCongViec->ghi_chu);
            $activeSheet->setCellValue("D12", "Ngày: ".date("d/m/Y", strtotime($trangThaiCongViec->created)));
            $activeSheet->setCellValue("D13", "Ký tên: ".$trangThaiCongViec->user->hoten);
        }
        $phongBanThamVan = PhongBanThucHienThamDinh::find()
            ->andFilterWhere(['phan_loai' => PhongBanThucHienThamDinh::THAM_DINH])
            ->andFilterWhere(['thuc_hien_cv_id' => $data->id])
            ->all();
        $dong = 16;
        /** @var PhongBanThucHienThamDinh $item */
        foreach ($phongBanThamVan as $item) {
            $activeSheet->setCellValue("A{$dong}", $item->phongBan->name.': '.$item->trang_thai);
            $activeSheet->insertNewRowBefore($dong+1);
            $dong++;
        }
        $dong+=3;
        $ykienLanhDao = ThucHienCvXinYKienLanhDao::findOne(['thuc_hien_cv_id' => $data->id]);
        if(!is_null($ykienLanhDao)){
            $activeSheet->setCellValue("A{$dong}", $ykienLanhDao->trang_thai." - ".$ykienLanhDao->ghi_chu);
            $activeSheet->setCellValue("D{$dong}", "Ngày: ".date("d/m/Y", strtotime($ykienLanhDao->created)));
            $activeSheet->setCellValue("D".($dong + 1), "Họ tên: ". $ykienLanhDao->lanhDao->hoten);
        }
//        foreach ($phongBanThucHien as $index => $item) {
//            $activeSheet->setCellValue("")
//        }
//        formatExcel::setBorder($activeSheet, "A8:I".($dong-1));
    }

    public function run()
    {
        $objReader = \PHPExcel_IOFactory::createReader('Excel2007');
        $objPHPExcel = $objReader->load(dirname(dirname(__DIR__)).'/common/template/MAU_PHIEU_DE_XUAT.xlsx');
        $activeSheet = $objPHPExcel->getActiveSheet();
        $this->renderBody($activeSheet);
        $this->filename = "DE_XUAT_CONG_VIEC_".$this->tieu_de.'.xlsx';
