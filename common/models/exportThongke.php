...
class exportThongke
{
    public $dulieu_thongke_theodonvi;
    public $tieude;
    public $thoigian;
    public $luachon_nhieudapan;
    public $mucdohailong;
    ...
    public $title = 'THONG KE KHAO SAT';
    public $subject = 'THONG KE KHAO SAT';
    public $description = '';
    public $category = '';
    public $lastModifiedBy = 'MINH HIEN SETE.,CO.LTD';
    public $keywords = '';
    public $sheetTitle = 'THONG KE KHAO SAT';
    public $legal = 'THONG KE KHAO SAT';
...
//        PHPExcel_Worksheet_PageSetup
        ...
        self::$objPHPExcel->createSheet()->setTitle("THONG KE KHAO SAT");
        self::$objPHPExcel->setActiveSheetIndexByName("THONG KE KHAO SAT");
    ...
    public function renderBody($activeSheet){
        $activeSheet->setCellValue("A2", $this->tieude);
        $activeSheet->setCellValue("A3", $this->thoigian);
        $activeSheet->setCellValue("A5", $this->mucdohailong);
        $dong = 8;
        foreach ($this->dulieu_thongke_theodonvi as $index => $item) {
            $activeSheet->setCellValue("A{$dong}", $index + 1);
            $activeSheet->setCellValue("B{$dong}", $item['mauphieu']);
            $activeSheet->setCellValue("C{$dong}", $item['name']);
            $activeSheet->setCellValue("D{$dong}", $item['tyle']);
            $arr_cot = ['E', 'F', 'G', 'H', 'I'];
            foreach ($item['ketquacaucuoi'] as $index2 => $item2){
                $activeSheet->setCellValue($arr_cot[$index2].$dong, $item2['tile']*100);
            }
            $dong++;
        }
    ...
    public function run()
    {
        ...
        $objPHPExcel = $objReader->load(dirname(dirname(__DIR__)).'/common/template/MAU_THONG_KE_KHAO_SAT.xlsx');
        $activeSheet = $objPHPExcel->getActiveSheet();
        $this->renderBody($activeSheet);
        $activeSheet->setTitle("KQ_THONG_KE");
        $this->filename = "KQ_THONG_KE ".$this->tieude;
