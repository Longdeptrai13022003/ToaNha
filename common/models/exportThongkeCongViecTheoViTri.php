...
class exportThongkeCongViecTheoViTri
{
    public $tieu_de;
    public $thoi_gian;
    public $data_cvtheovtcv;
    public $data_cvphatsinh;
    public $phong_ban;
    public $ho_ten;
    public $path_file;
    ...
    public $title = 'THONG KE CV THEO VI TRI CV';
    public $subject = 'THONG KE CV THEO VI TRI CV';
    public $description = '';
    public $category = '';
    public $lastModifiedBy = 'MINH HIEN SETE.,CO.LTD';
    public $keywords = '';
    public $sheetTitle = 'THONG KE CV THEO VI TRI CV';
    public $legal = 'THONG KE CV THEO VI TRI CV';
...
//        PHPExcel_Worksheet_PageSetup
        ...
        self::$objPHPExcel->createSheet()->setTitle("THONG KE CV THEO VI TRI CV");
        self::$objPHPExcel->setActiveSheetIndexByName("THONG KE CV THEO VI TRI CV");
    ...
    public function renderBody($activeSheet){
        ...
        $dong = 12;
        foreach ($this->data_cvtheovtcv as $index => $item) {
            $activeSheet->setCellValue("A{$dong}", $index + 1);
...
//            $activeSheet->setCellValue("G{$dong}", date("Y", strtotime($item['ngay_thang_giao_viec'])));
            ...
            $tongDiemKeHoach += floatval($item['diem_so']);
            $tongDiemCaNhan += floatval($item['boss_danh_gia']);
            $dong++;
        }
        $activeSheet->setCellValue("A{$dong}", "II");
        $activeSheet->setCellValue("B{$dong}", "Công việc phát sinh");
        $activeSheet->mergeCells("B{$dong}:N{$dong}");
        formatExcel::setFontBold($activeSheet, "A{$dong}:B{$dong}");

        $dong++;

        foreach ($this->data_cvphatsinh as $index => $item) {
            $activeSheet->setCellValue("A{$dong}", $index + 1);
            $activeSheet->setCellValue("B{$dong}", $item['nguoi_phan_cong']);
//            $activeSheet->setCellValue("C{$dong}", $item['phong_ban_thuc_hien']);
            $activeSheet->setCellValue("C{$dong}", $item['cong_viec_thuc_hien']);
...
//            $activeSheet->setCellValue("G{$dong}", date("Y", strtotime($item['ngay_thang_giao_viec'])));
        ...
        $activeSheet->setCellValue('A'.$dong, 'TỔNG ĐIỂM');
        $activeSheet->setCellValue('K'.$dong, $tongDiemKeHoach);
        $activeSheet->setCellValue('L'. $dong, $tongDiemCaNhan);
        $activeSheet->mergeCells("A{$dong}:J{$dong}");
        formatExcel::setBorder($activeSheet, 'A5:M'.$dong);
    }
    ...
    public function run()
    {
        ...
        $this->filename = 'BAO_CAO_CONG_VIEC_THEO_VTCV_'.myAPI::createCode($this->ho_ten).'-'.time().'.xlsx';;
